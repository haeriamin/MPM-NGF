      ...
      for (int p_i = particle_begin; p_i < particle_end; p_i++) {
        ...
        for (int node_id = 0; node_id < Cache::num_nodes; node_id++) {
          ...
          if ((grid_state & mask) != (particle_state & mask)) {
            ...
            // due to leakage and sub-grid issues
            // if (p.boundary_distance <= delta_x/2.0_f)
            // if (p.boundary_distance >= 0.0_f)
            // {
              // added: Force and torque on rigid bodies' center of mass
              // TODO: modify it
              Vector impulse_tmp = impulse / delta_t;
              r->rigid_force_tmp += impulse_tmp;
              r->rigid_torque_tmp += cross(delta_x * grid_pos[node_id] - r->position, impulse_tmp);

              // added: Impulse on rigid body boundary particles
              // TODO: Only valid grid nodes should affect rigid particles
              for (int r_p_i = particle_begin; r_p_i < particle_end; r_p_i++)
              {
                Particle &r_p = *allocator[particles[r_p_i]];
                if (r_p.is_rigid())
                {
                  const Vector r_pos = r_p.pos * inv_delta_x;
                  Kernel r_kernel(r_pos, inv_delta_x);
                  const VectorP(&r_kernels)[3][3][3] = r_kernel.kernels;
                  using KernelLinearized = VectorP[27];
                  const KernelLinearized &r_kernels_linearized =
                    *reinterpret_cast<const KernelLinearized *>(&r_kernels[0][0][0]);
                  const VectorP &r_dw_w = r_kernels_linearized[node_id];
                  r_p.rigid_impulse += impulse * r_dw_w[dim];
                }
              }
            // }
            ...
            // added: Due to incompatible grid node
            // gf = 0.0_f;  // ------------------------------ ????????
          }
