  ...
  auto block_op_rigid = [&](uint32 b, uint64 block_offset, GridState<dim> *g_) {
    ...
    for (uint32 t = 0; t < SparseMask::elements_per_block; t++) {
      ...
      for (int p_i = particle_begin; p_i < particle_end; p_i++) {
        ...
        // Note, pos is magnified (0-res) particle pos
        const Vector pos = p.pos * inv_delta_x;
        ...
        if (p.p > 0.0_f)  // pressure @ n
          gf = p.gf;
        Matrix delta_t_tmp_force = delta_t * p.calculate_force();
        ...
        for (int node_id = 0; node_id < Cache::num_nodes; node_id++) {
          ...
          // only for compatible grid nodes
          if ((grid_state & mask) != (particle_state & mask)){
            continue;
          }
        ...
        {
          GridState<dim> &gr =
            grid_cache.linear[grid_cache.kernel_linearized(13)+grid_cache_offset];
          RigidBody<dim> *r = get_rigid_body_ptr(gr.get_rigid_body_id());
          if (r == nullptr)
            continue;
          ...
          if (p.near_boundary() && p.boundary_distance <= dist)
          {
            ...
            Vector velocity_change =
              ...
              friction_project(
                v_acc,
                r->get_velocity_at(delta_x * grid_pos[13]),
                p.boundary_normal,
                r->frictions[(p.states >> (2 * r->id)) % 2]
              );
            real w1 = (dist-abs(p.boundary_distance))/dist;
            Vector impulse = w1 * mass * velocity_change
                             + delta_t_tmp_force * Vector(w1);  // ?
              
            // Force and torque on rigid bodies' center of mass
            Vector impulse_tmp = impulse / delta_t;
            r->rigid_force_tmp += impulse_tmp;
            r->rigid_torque_tmp += cross(delta_x * grid_pos[13] - r->position, impulse_tmp);
            ...
            if (config_backup.get("visualize_particle_impulses", false))
            {
              for (int r_p_i = particle_begin; r_p_i < particle_end; r_p_i++)
              {
                ...
                if (r_p.is_rigid()){
                  ...
                  real w2 = sqrt(pow(dpos2[0],2)+pow(dpos2[1],2)+pow(dpos2[2],2)) / sqrt(2);
                  r_p.rigid_impulse += impulse * w2;
                }
              }
            }
...
void MPM<3>::resample_optimized() {
  ...
  auto block_op_rigid = [&](uint32 b, uint64 block_offset, GridState<dim> *g) {
    ...
    for (uint32 t = 0; t < SparseMask::elements_per_block; t++) {
      ...
      real laplacian_gf =
        this->inv_delta_x * this->inv_delta_x * (
      ...
      for (int k = particle_begin; k < particle_end; k++) {
        ...
        for (int node_id = 0; node_id < Cache::num_nodes; node_id++) {
          ...
          // added
          if (((grid_state & mask) != (particle_state & mask)) && p.near_boundary()){
            grid_vel = p.get_velocity();
            if (p.boundary_distance / (0.05_f * delta_x) <= 1.0_f)
              laplacian_gf = 0.0_f;
          }
          else 
          if ((grid_state & mask) != (particle_state & mask))
        ...
        // added: Modified rb interaction approach
        real dist = p.boundary_distance / (0.05_f * delta_x);
        if (p.near_boundary() &&  dist <= 1.0_f)
        {
          ...
          // p.set_velocity(dist*p.get_velocity() + v_collision);
          p.set_velocity(v_collision);

          // // b can be updated here for better accuracy:
          // Vector w_grid_vel = (1.0_f-dist) * v_collision;
          // for (int r = 0; r < dim; r++) {
          //   b[r]   = fused_mul_add(w_grid_vel, Vector(dpos[r]), b[r]);
          //   cdg[r] = fused_mul_add(v_collision, Vector(1.0_f-dist), cdg[r]);
          // }
        }
