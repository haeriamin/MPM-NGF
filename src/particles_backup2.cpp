 ...
 public:
  ...
  int plasticity(const Matrix &cdg, const real &laplacian_gf) override {
    ...
    auto kinematics = [I](auto cdg, auto delta_t) {
      ...
      Matrix D_0 = D - D.trace()/3.0_f * I;  // Deviatoric part of D
    ...
    if (p_n > 0.0_f){  // p @ n
      ...
      // real gdot_loc = - ((mu_2 - mu_s) * (mu_s - mu) / (mu_2 - mu) * this->gf)
      ...
      // ** Zero gf is an unstable solution
      this->gf = std::max(eps,  // gf @ n+1
                  (delta_t * (gdot_loc + gdot_nonloc) / t_0) + this->gf);

    } else if (p_n == 0.0_f){  // p @ n
      this->gf = std::max(eps, kinematics(cdg, delta_t) / mu_2);
    }
    ...
    // Disconnected
      ...
      // Flow direction
      // Matrix L = (1.0_f / delta_t) * (cdg - I);
      // real L_mag = 0.0_f;
      // for (int i = 0; i < dim; ++i)
      //   for (int j = 0; j < dim; ++j)
      //     L_mag += pow(L[i][j], 2.0_f);
      // Np = (1 / sqrt(L_mag)) * L;

      if (p_n > 0.0_f)
        this->gf = std::max(eps, kinematics(cdg, delta_t) / mu_2);

      // this->dg_p = (I + delta_t * 0.707_f * mu * this->gf * Np) * this->dg_p;
    ...
    // Dense
      ...
      if (this->tau > tau_trial || tau_trial <= mu_s*this->p)
        this->tau = tau_trial;
      ...
      this->T = Re * Me * transpose(Re);  // T @ n+1
