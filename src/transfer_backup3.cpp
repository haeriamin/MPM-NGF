    ...
    for (uint32 t = 0; t < SparseMask::elements_per_block; t++) {
      ...
      for (int p_i = particle_begin; p_i < particle_end; p_i++) {
        ...
        if (config_backup.get("compute_particle_impulses", false))
        {
          ...
          // real dist = 0.1_f * delta_x;
          // if (p.near_boundary() && p.boundary_distance <= dist)
          // {
            Vector v_acc = v;  // + apic_b_inv_d_mass * dpos / Vector(mass);
            // Vector com_r = (pos*delta_x - r->position) - p.boundary_distance*p.boundary_normal;
            Vector velocity_change =
              v_acc -
              friction_project(
                v_acc,
                r->get_velocity_at(delta_x * grid_pos[13]),
                // r->get_velocity_at(com_r+r->position),
                p.boundary_normal,
                r->frictions[(p.states >> (2 * r->id)) % 2]
              );
            // real w1 = (dist-min(p.boundary_distance,0.0_f))/dist;
            // force = volume * (div . stress)
            Vector impulse = mass * velocity_change +
                             delta_t_tmp_force * Vector(1.0_f);
              
            // Force and torque on rigid bodies' center of mass
            Vector force_tmp = impulse / delta_t;
            // Vector force_tmp = w1 * impulse / delta_t;
            r->rigid_force_tmp += force_tmp;
            // r->rigid_torque_tmp += cross(com_r, force_tmp);
            r->rigid_torque_tmp += cross(delta_x * grid_pos[13] - r->position, force_tmp);
