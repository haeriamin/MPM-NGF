...
if __name__ == '__main__':
    ...
    finalTime = 20  # sec
    frameRate = 60  # Hz
    Scale = 1  # problem scale; less particles more jelly-like behavior, less number of DOF
    oneG = 9.81
    micG = .02*oneG
    ...
    offset = 0.2
    length_bin = 0.25*2 * Scale
    height_bin = 0.25 * Scale
    width_bin = length_bin/4
    orifice = 0.01 * min(1,Scale*2)
    ...
    walls_pos = [
        # top floor left
        tc.Vector(
            offset + length_bin/4 - orifice,
            offset + height_bin,
            offset + width_bin/2),
        # top floor right
        tc.Vector(
            offset + length_bin*3/4 + orifice,
            offset + height_bin,
            offset + width_bin/2),
        # left wall
        tc.Vector(
    ...
    walls_rot = [
        tc.Vector(0, 0, 0),   # top floor left
        tc.Vector(0, 0, 0),   # top floor right
    ...
    extra = 0.005
    walls_scale = [
        (.25*Scale, 1, (.125+extra)*Scale), 
        (.25*Scale, 1, (.125+extra)*Scale), 
        (.15*Scale, 1, .15*Scale),
        (.15*Scale, 1, .15*Scale),
        ((.25*2+extra)*Scale, 1, .15*Scale),
        ((.25*2+extra)*Scale, 1, .15*Scale),
    ]
    ...
    Y_mod = 1e6  # lower this looser sand; up to some value due to large Me_tr! (has issue from state 2 to state 3)
    nu = 0.3
    mpm.add_particles(
        type='nonlocal',
        pd=True,
        ...
        t_0=1e-3, #1e-3 makes g go to infinity because of soil position
    )
